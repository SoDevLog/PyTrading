...
def plot_bsi(ticker: str = "AAPL",
    ...
    ax_hist_ct.set_ylabel("BSI CT 9/26", fontsize=9, color=T["text_muted"])
    ...
    ax_hist_mt.set_ylabel("BSI MT 21/55", fontsize=9, color=T["text_muted"])

    # -- Histogramme BSI LT — MACD 50/200/9 (long terme, la boussole) -------------
    colors_lt = [T["bull"] if v >= 0 else T["bear"] for v in hist_lt["hist"]]
    ax_hist_lt.bar(idx, hist_lt["hist"], color=colors_lt, width=0.6)
    ax_hist_lt.axhline(0, color=T["zero_line"], lw=0.5)
    ax_hist_lt.set_ylabel("BSI LT 50/200", fontsize=9, color=T["text_muted"])
