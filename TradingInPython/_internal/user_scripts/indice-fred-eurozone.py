"""
MACRO ISM — Zone Euro (FRED)
============================
Indice synthétique de santé économique de la Zone Euro,
construit sur le même modèle que la version US,
à partir de séries disponibles sur FRED.

Différences vs version US :
  - Toutes les séries sont eurozonaires (OCDE / Eurostat / BCE via FRED)
  - Certaines séries OCDE sont trimestrielles → interpolation mensuelle automatique
  - HICP remplace CPI
  - M3 remplace les commandes (pas d'équivalent mensuel durable pour la ZE)
  - Confiance consommateur OCDE remplace Univ. Michigan

Séries utilisées :
  PRINTO01EZQ661S  Production industrielle ZE (trimestrielle → interpolée)
  LRHUTTTTEZM156S  Taux de chômage harmonisé ZE (mensuelle)
  CP0000EZ19M086NEST  HICP inflation ZE (mensuelle)
  SLRTTO01EZQ659S  Ventes au détail volume ZE (trimestrielle → interpolée)
  CSCICP03EZM665S  Confiance consommateur OCDE ZE (mensuelle)
  MABMM301EZM657S  M3 masse monétaire ZE (mensuelle)
"""
...
# ── Séries Zone Euro disponibles sur FRED ─────────────────────────────────────
#
...
print(f"[{END}] Téléchargement des séries FRED — Zone Euro…")
raw = {}
...
# Certaines séries OCDE sont trimestrielles ; on les interpole en mensuel.

df_raw = {}
for ticker, series in raw.items():
    # Resample en mensuel (début de mois), puis interpolation linéaire temporelle
    df_raw[ticker] = (
        series
        .resample('MS').interpolate('time')
        .interpolate('time')           # comble les NaN résiduels
    )
...
# )
# df[export_cols].to_csv('macro_ism_eurozone.csv')
# print("Export CSV → macro_ism_eurozone.csv")
...
print(f"""
╔══════════════════════════════════════════════════╗
║      MACRO ISM ZONE EURO — DERNIER POINT         ║
╠══════════════════════════════════════════════════╣
...
ax_main.plot(df.index, df['MACRO_ISM_S'], color='#003399', lw=2.2, label=f'Lissé ({SMOOTH_LONG}m)')
...
ax_main.set_title('Macro ISM Zone Euro — Indice composite pondéré (FRED/OCDE/Eurostat)',
...
ax_rec.fill_between(df.index, df['RECESSION_PROB'], alpha=0.6, color='#003399')
ax_rec.axhline(0.5, ls='--', color='black', lw=1)
ax_rec.set_ylim(0, 1)
ax_rec.set_title('Probabilité de récession ZE (sigmoïde)', fontsize=10)
...
fig.suptitle(f'Macro ISM Zone Euro | Données FRED | Généré le {END}',
             fontsize=11, style='italic', color='#555')

# plt.savefig('macro_ism_eurozone.png', dpi=150, bbox_inches='tight')
# print("Export graphique → macro_ism_eurozone.png")
plt.show()